...
//     1000
//   ),
//   createData("آجل", "Eclair", 12, "شركة مدعوم للمنتجات الطبية", 2000),
...
export const SalesRecord = () => {
  ...
  const getInvoices = async () => {
    try {
      ...
      const salesArr = data.data.filter((obj) => obj.invoiceType === "SELLING");
      setInvoicesArr([...salesArr]);
      setFilteredArr([...salesArr]);
      setFilteredArr2([...salesArr]);
      // message.success("invoices found");
    } catch (e) {
      //alert("invoices not found");
      // message.error("Invoices not found");
      //e.message.error("username or password is incorrect");
    }
    ...
    <MainLayout>
      <div className="SRwrapper">
        <Container className="containerDiv">
          <div className="SRheaderDiv">
            <div className="SRtitleDateDiv">
              <div className="SRtitle">
                <p className="SRtitleTxt">سجل فواتير البيع</p>
              </div>
              <Search
              ...
              <TableBody>
                ...
                {invoicesArr.map((row) => (
                  <TableRow
                    ...
                    className="SRtableRow"
                  >
                    <CustomCell className="SRtableCell" align="center">
                      {/* {row.name} */}
